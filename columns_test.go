...
func TestColumnsCellAreas(t *testing.T) {
	...
	tui.Columns(Split().Fixed(5).Factor(1), cell, cell)
...
func TestColumnsAdvanceCursorToTallestColumn(t *testing.T) {
	...
	tui.Columns(Split().Factor(1, 1),
		func() { tui.moveCursor(mathi.Vec2{Y: 3}) },
		func() { tui.moveCursor(mathi.Vec2{Y: 5}) },
	)
...
func TestColumnsPanicsOnCellCountMismatch(t *testing.T) {
	...
	tui.Columns(Split().Factor(1, 1), func() {})
}
